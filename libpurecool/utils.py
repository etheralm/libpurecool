...
def is_pure_hot_cool_link_v2(product_type):
    """Return True if it is a v2 dyson pure hot cool link device.

    :param product_type Dyson device model
    """
    if product_type in [DYSON_PURE_HOT_COOL_LINK]:
        return True
    return False
...
def is_dyson_pure_hot_cool_link_device(json_payload):
    """Return true if this json payload is a v2 dyson pure hot cool link device."""
    if json_payload['ProductType'] in [DYSON_PURE_HOT_COOL_LINK]:
        return True
    return False
